...
export class Solution {
  struct Node {
    ...
    constexpr Node(long long value) : value(value), sum(value) {}
  };
  ...
  static void expose(Node *node) {
    while (true) {
      ...
      maintain_invariant(path_parent);
    }
  }
  ...
  std::optional<long long> sum(Node *u, Node *v) {
    if (!is_connected(u, v))
      return std::nullopt;
    splay(v);
    return v->sum;
...
public:
  ...
  dynamicPathSum(const std::vector<long long> &values,
    ...
    std::vector<Node> nodes =
        values |
        std::views::transform([](long long value) { return Node{value}; }) |
        std::ranges::to<std::vector>();
    std::vector<std::optional<long long>> res;
    for (auto [kind, u, v] : operations) {
      ...
      case 2:
        ...
        break;
      case 3:
        res.push_back(sum(&nodes[u], &nodes[v]));
        break;
      default:
        std::abort();
      }
