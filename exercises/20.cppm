...
export class Solution {
  struct Node {
    ...
    constexpr Node(long long value = 0) : value(value), sum(value) {}
  };
  ...
  public:
    ...
    Graph(const std::vector<long long> &values) : handles(values.size()) {
      nodes =
          values |
    ...
    void set(std::size_t u, long long value) {
      Node *node = &nodes[u];
      splay(node);
      node->value = value;
      maintain_invariant(node);
    }
    long long sum(std::size_t u) {
      Node *node = &nodes[u];
      splay(node);
      return node->sum;
    }
...
public:
  ...
  componentSum(const std::vector<long long> &values,
