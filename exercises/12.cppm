...
export class Solution {
  ...
  public:
    ...
    int get_depth(int x) const { return depth[x]; }
...
public:
  std::vector<int> rerootedLca(const std::vector<int> &parent,
                               const std::vector<std::array<int, 3>> &queries) {
    Table table(parent);
    return queries | std::views::transform([&](std::array<int, 3> queries) {
             std::array<int, 3> lcas = {table.get_lca(queries[0], queries[1]),
                                        table.get_lca(queries[0], queries[2]),
                                        table.get_lca(queries[1], queries[2])};
             return std::ranges::max(
                 lcas, {}, [&](int node) { return table.get_depth(node); });
           }) |
           std::ranges::to<std::vector>();
  }
};
