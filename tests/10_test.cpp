...
TEST(Stage10, SmallOracleAndEdgeCases) {
  Solution solution;
  ...
  for (int n : {1, 2, 7, 31}) {
    ...
    for (auto [l, r] : queries)
      expected.push_back(std::min_element(a.begin() + l, a.begin() + r + 1) -
                         a.begin());
    EXPECT_EQ(solution.argMin(a, queries), expected);
    EXPECT_TRUE(solution.argMin(a, {}).empty());
  }
}

TEST(Stage10, ScaleAndShape) {
  Solution solution;
  const int n = 120000;
  std::vector<long long> a(n);
