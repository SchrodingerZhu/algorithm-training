# 第二组：RMQ → 倍增与 LCA → 树链剖分 → LCT → ETT

这是独立于筛法的树与区间查询题组，共 22 道主线题，另有 10-a、10-b 两道扩展题。当前分支 `rmq-lca-lct`；筛法与原代码在 `sieves` 分支。先做 01，再逐题推进。已经熟练的基础题可通过测试与解释不变量快速跳过。
...
| 19 | 从零实现 ETT：动态森林连通性 | ETT | [题面与提示](lessons/19.md) | [练习](exercises/19.cppm) |
| 20 | ETT 扩展：连通分量点权和 | ETT | [题面与提示](lessons/20.md) | [练习](exercises/20.cppm) |
| 21 | 独立练习：目录树的挂接与拆分 | ETT | [题面与提示](lessons/21.md) | [练习](exercises/21.cppm) |
| 22 | 独立练习：部门收支与拆分查询 | ETT | [题面与提示](lessons/22.md) | [练习](exercises/22.cppm) |
...
**ETT 从零开始的路线**：19 直接从动态森林连通性出发，先手算游走、换根、连断边，再实现所需的 split/merge、rank 与 root；20 只增加点赋值和分量和。21、22 是独立应用题，题面完整给出操作语义与边界，解法放在可展开的提示中：21 维护有根目录树，22 查询部门收支及拆分后一侧的收支。19～22 均有 TODO 模板与测试，可用 `./judge 19` 等逐题运行。
